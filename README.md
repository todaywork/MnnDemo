# OnDeviceAi SDK Demo（v1.0.11）

`sdk_demo` 用于演示和验证 `on_device_ai` AAR 的模型初始化、文本生成、流式回调、性能指标和图片输入能力。

## 版本迭代

- **v1.0.11（当前版本）**：升级 MNN 工具链及 3.6.1 Runtime，解决新模型导出后的加载兼容问题，并支持新版结构化 `tie_embeddings` 模型。
...
```groovy
implementation files("../on_device_ai/build/outputs/aar/on_device_ai-1.0.11-release.aar")
```
